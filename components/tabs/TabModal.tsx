      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
          ...
          <motion.div
            ...
            className="relative z-10 bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-zinc-200 space-y-5"
          >
            ...
            {/* Tab Type Switcher with Animated Sliding Pill */}
            <div className="relative flex p-1 bg-zinc-100 rounded-xl border border-zinc-200">
              <button
                ...
                className={`relative z-10 flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${
                  type === 'owed_to_you' ? 'text-white' : 'text-zinc-600 hover:text-black'
                }`}
              >
                {type === 'owed_to_you' && (
                  <motion.div
                    layoutId="tabModalTypePill"
                    className="absolute inset-0 bg-black rounded-lg shadow-xs"
                    transition={{ type: 'spring', stiffness: 450, damping: 35 }}
                  />
                )}
                <ArrowUpRight className="w-3.5 h-3.5 relative z-10" />
                <span className="relative z-10">Someone Owes Me</span>
              </button>
              <button
                type="button"
                onClick={() => setType('you_owe')}
                className={`relative z-10 flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-colors cursor-pointer ${
                  type === 'you_owe' ? 'text-white' : 'text-zinc-600 hover:text-black'
                }`}
              >
                {type === 'you_owe' && (
                  <motion.div
                ...
                <span className="relative z-10">I Have to Give Back</span>
              </button>
            </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3.5 py-2.5 bg-zinc-50 border border-zinc-200 rounded-xl text-xs font-medium text-zinc-900 focus:outline-none focus:ring-2 focus:ring-black focus:bg-white transition-colors"
                  />
                </div>
              </div>
